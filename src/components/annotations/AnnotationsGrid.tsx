...
import { AnnotationCardsSkeleton } from './AnnotationsSkeleton';
...
export const AnnotationsGrid = ({
  ...
  if (annotations.length === 0) {
    return <Box aria-busy={isLoading}>{isLoading ? null : <EmptyState />}</Box>;
  }
        ...
        <>
          <Grid container spacing={3}>
            {annotations.map(annotation => (
              <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={`${annotation.source}:${annotation.uuid}`}>
