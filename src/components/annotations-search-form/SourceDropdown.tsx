  ...
  return (
    <li key={key} {...optionProps}>
      <Checkbox
        ...
        disabled={isDisabled}
      />
      <Typography sx={{ flex: 1, color: isDisabled ? 'text.disabled' : 'text.primary' }}>
        {option.label}
      </Typography>
...
export const SourceDropdown = ({
  ...
  useEffect(() => {
    if (data?.results) {
      const formattedOptions = data.results.map(source => ({
      ...
      const healthySources = formattedOptions.filter(o => o.status === 'ok').map(o => o.value);
  ...
  const allSelected =
    enabledOptions.length > 0 && enabledOptions.every(o => selectedSources.includes(o.value));
  ...
  const handleAllToggle = () => {
    onSelectedSourcesChange(allSelected ? [] : enabledOptions.map(o => o.value));
  };
  ...
  return (
    <Autocomplete
      ...
      renderOption={(props, option, { selected }) => {
        ...
        return (
          <SourceOptionItem key={option.value} props={props} option={option} selected={selected} />
        );
      }}
      renderInput={params => (
