...
describe('apiRequest', () => {
  it('returns response data on a successful GET request', async () => {
    mock
      .onGet('/sources')
      .reply(200, { sources: [{ source_name: 'bodc' }, { source_name: 'jncc' }] });
  ...
  it('sends query params correctly, including repeated array values', async () => {
    mock.onGet('/taxonomy/worms/taxa/crab').reply(config => {
  ...
  it('throws on a server error response', async () => {
    ...
    await expect(apiRequest({ method: 'GET', url: '/sources' })).rejects.toMatchObject({
      response: { status: 500 }
    });
  });
