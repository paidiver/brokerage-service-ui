...
export function useApiRequest<T>() {
  ...
  const makeRequest = useCallback(
    async (requestOptions: RequestOptions & { isEmpty?: (data: T) => boolean }) => {
      ...
      try {
        ...
        setState({
          data: null,
          status: statusCode === 400 || statusCode === 422 ? 'validationError' : 'serverError',
          error:
            statusCode === 400 || statusCode === 422
              ? (axiosError.response?.data?.detail ?? 'Invalid request parameters.')
